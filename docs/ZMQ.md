# The Current/Future Status of ZMQ in Bonero

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Bonero
...
matching supported by both Bonero and ZMQ. The `format`, `context` and `event`
...
The Bonero daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Bonero daemon also ensures that
...
The Bonero daemon will send a `txpool_add` pub exactly once for each
