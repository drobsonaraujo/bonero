...
used by Bonero. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Bonero's
...
> if interested in learning about Bonero p2p traffic!


## Header
This header is sent for every Bonero p2p message.
...
Bonero when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Bonero specific
...
### Requests
Requests are the basis of the admin protocol for Bonero. The `Q` bit must be
