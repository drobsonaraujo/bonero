...
Core tests take longer than any other Bonero tests, due to the high amount of computational work involved in validating core components.
...
To run only Bonero's core tests (after building):
...
To run only Bonero's crypto tests (after building):
...
To run only Bonero's hash tests (after building):
...
To run only Bonero's performance tests (after building):
...
To run only Bonero's unit tests (after building):
